...
test('Integrations workspace is capability gated and exposes a concrete Jira Assets federated-read UI', async () => {
  ...
  for (const id of ['jira-assets-connectors', 'jira-assets-search', 'jira-assets-connector', 'jira-assets-aql', 'jira-assets-results']) {
...
test('Integrations workspace source only performs governed reads through JiraAssetsClient', async () => {
  ...
  assert.match(source, /client\.search\(connector, aql/);
  assert.doesNotMatch(source, /client\.(create|update|delete|import|synchronize|push)\(/);
});
