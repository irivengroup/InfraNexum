# InfraNexum 2.0.0-alpha.0.108 — PGM-10-E06 phase 1: Jira Assets federated read

**Statut : EN COURS / NON TERMINÉ. Nature : évolution fonctionnelle bornée.** Cette version ouvre PGM-10-E06 par une première intégration officielle Jira Assets en **lecture fédérée sans copie**. PGM-10-E05 reste fonctionnellement présent mais n'est toujours pas clôturé formellement : les gates hébergés Temurin 25/Maven/JaCoCo et PostgreSQL 17/18 restent obligatoires. PGM-10-E06 complet reste également NON TERMINÉ tant que ServiceNow, OpenService, notifications et les tranches de synchronisation/rollback prévues par la roadmap ne sont pas livrés.

**Jira Assets / autorité :** `components.adapters.jira-assets` est un adapter Java natif autonome, sans dépendance RSOT/ITAM. Le flux est déclaré `FEDERATED_READ`, autorité `EXTERNAL` : InfraNexum interroge Jira Assets à la demande et ne transforme aucune valeur distante en donnée canonique locale. La health probe lit le catalogue de schémas ; la recherche utilise l'API AQL POST courante avec pagination bornée et `includeAttributes=false`, de sorte que la tranche ne rapatrie que l'identité/type minimale des objets.

**Sécurité / résilience :** egress HTTPS limité à `api.atlassian.com`, redirections refusées, timeouts <= 60 s, réponses bornées (2 MiB par défaut, maximum configurable 8 MiB), AQL <= 4 096 caractères, pagination `offset <= 1 000 000` / `limit <= 200`, erreurs fournisseur traduites sans recopier leur payload. Le bearer token est résolu uniquement depuis `env:` ou `file:`, utilisé côté Server puis son buffer est effacé ; aucun token, `cloudId`, `workspaceId` ou secret n'est publié au navigateur. La configuration produit par défaut contient zéro connecteur Jira.

**API/Web :** trois opérations Server réelles sont ajoutées : catalogue de connecteurs Jira Assets paginé, health d'un connecteur et recherche AQL paginée. Elles sont toutes capability-gated par `integrations.connectors` et autorisées par `integrations.connector.read`. Le Web ajoute une entrée Integrations uniquement lorsque la capability est publiée et expose un workspace Jira Assets lecture seule : liste, health, formulaire AQL et résultats. Les cinq locales DE/EN/ES/FR/IT sont couvertes.

**Qualification locale :** **EXÉCUTÉ** — API Contracts **48/48**, couverture **98 %**, **15 fragments / 182 opérations, dette 0/0/0/0** ; Architecture fonctionnelle **177/177** en split, méta-checker **29/29** exécuté indépendamment et Architecture-as-Code `PASS`, dont pagination **5/5**, gouvernance authorization/capability **6/6** et invariants Jira Assets **6/6** ; Web **202/202** sous Node **22.16.0**, couverture **99,73 % lignes / 98,54 % branches / 100 % fonctions**, smoke process réussi ; Migrations **117/117**, Eventing **10/10**, Persistence **12/12**, Capabilities **10/10**, Entitlements **10/10**, Audit **8/8**, Toolchains **25/25**, SDK connecteurs **19/19** et contrats Compose **68/68**. Les smokes Java dependency-free Integrations, Policy et API Capability sortent à zéro sous Java **21.0.11**. Le snapshot de packaging contient **1 409 chemins canoniques / 1 411 fichiers suivis / 1 410 empreintes Git** ; Source Integrity strict et Archive Compatibility sont les derniers gates exécutés sur le commit de packaging avant émission de l'archive.

**NON EXÉCUTÉ / gates externes :** Temurin **25.0.4+7** avec Maven/JUnit/JaCoCo et seuil **98 % lignes + branches**, dont les JUnit du nouvel adapter Jira Assets ; PostgreSQL **17/18 live** ; Docker Desktop/Compose PRO et wrapper PowerShell 7 ; Node **24.18.1/pnpm 11.17.0** exact ; Go **1.26.5** (le Go 1.23.2 local tente son téléchargement mais le runner est hors réseau) ; navigateur réel ; appel Jira Assets réel avec un tenant/workspace/token autorisé. Aucun de ces contrôles n'est remplacé par un mock ou déclaré exécuté localement.

Voir `docs/integrations-jira-assets.md`.
