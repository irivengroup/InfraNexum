...
The current renderer integration pins Swagger UI `5.32.13` and ReDoc CE `2.5.3`. Swagger retains its existing lazy-loaded `cdn.jsdelivr.net` presentation assets. ReDoc is different by design: its JavaScript is strictly self-hosted at `/assets/vendor/redoc/2.5.3/redoc.standalone.js`, with no runtime network fallback and a frame CSP restricted to `script-src 'self'`. The certified local OpenAPI document remains the source of truth. Swagger mutation submission is disabled in the embedded viewer; ReDoc receives InfraNexum light/dark palette tokens, re-renders when the product theme changes, and uses a persistent same-origin resize bridge so the page—not an internally clipped frame—owns vertical scrolling.
