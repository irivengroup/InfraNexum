...
class ArchitectureChecker:
    ...
    def _check_source_layout(self) -> None:
        """Require every implementation source file to live below the source root."""
        ...
        for path in self._repository_paths():
            ...
            if not path.is_relative_to(self.source_root):
                self._add(
                    "CHECK-ARCH-SRC-002",
                    path,
                    f"implementation source files must be located below {rendered_source_root!r}",
                )
