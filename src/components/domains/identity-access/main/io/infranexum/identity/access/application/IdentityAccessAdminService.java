...
public final class IdentityAccessAdminService {
    ...
    public OffsetPage<GroupMember> groupMembers(DomainIdentifier organizationId,DomainIdentifier groupId,int offset,int limit){ getGroup(Objects.requireNonNull(organizationId,"organizationId"),Objects.requireNonNull(groupId,"groupId")); PaginationConstraints.requireOffset(offset);page(offset,limit);List<GroupMember> rows=repository.groupMembers(organizationId,groupId,offset,limit+1);boolean more=rows.size()>limit;List<GroupMember> items=List.copyOf(rows.subList(0,Math.min(limit,rows.size())));return new OffsetPage<>(items,more?Math.addExact(offset,limit):null);}
