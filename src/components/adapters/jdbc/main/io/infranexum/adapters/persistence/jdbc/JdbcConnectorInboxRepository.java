...
public final class JdbcConnectorInboxRepository implements ConnectorInboxRepository {
    ...
    private boolean insertAdmission(Connection connection, WebhookAdmission admission) throws SQLException {
        String query = sql(
                "INSERT INTO infranexum_integrations.connector_inbox(delivery_id,connector_key,external_delivery_id,payload_json,payload_sha256,status,attempts,received_at,available_at,lease_owner,lease_until,processed_at,last_failure,replay_count,last_replayed_at,created_at,updated_at) VALUES(?,?,?,CAST(? AS JSONB),?,'PENDING',0,?,?,NULL,NULL,NULL,NULL,0,NULL,?,?) ON CONFLICT(connector_key,external_delivery_id) DO NOTHING",
                "INSERT INTO INFRANEXUM_INTEGRATION_INBOX(DELIVERY_ID,CONNECTOR_KEY,EXTERNAL_DELIVERY_ID,PAYLOAD_JSON,PAYLOAD_SHA256,STATUS,ATTEMPTS,RECEIVED_AT,AVAILABLE_AT,LEASE_OWNER,LEASE_UNTIL,PROCESSED_AT,LAST_FAILURE,REPLAY_COUNT,LAST_REPLAYED_AT,CREATED_AT,UPDATED_AT) VALUES(?,?,?,?,?,'PENDING',0,?,?,NULL,NULL,NULL,NULL,0,NULL,?,?)");
        Savepoint savepoint = dialect == JdbcDatabaseDialect.ORACLE ? connection.setSavepoint() : null;
        try (PreparedStatement statement = connection.prepareStatement(query)) {
            dialect.bindIdentifier(statement, 1, admission.deliveryId()); statement.setString(2, admission.connectorKey().value()); statement.setString(3, admission.externalDeliveryId()); dialect.bindJson(statement, 4, admission.payload()); statement.setString(5, admission.payloadSha256()); JdbcTemporal.bindInstant(statement, 6, admission.receivedAt()); JdbcTemporal.bindInstant(statement, 7, admission.receivedAt()); JdbcTemporal.bindInstant(statement, 8, admission.receivedAt()); JdbcTemporal.bindInstant(statement, 9, admission.receivedAt());
