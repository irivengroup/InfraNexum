...
public final class IntegrationController {
    ...
    @PostMapping(path="/api/v1/integrations/webhooks/{connectorKey}",consumes=MediaType.APPLICATION_JSON_VALUE,produces=MediaType.APPLICATION_JSON_VALUE)
    ResponseEntity<WebhookAdmissionResponse> admit(
        ...
        return ResponseEntity.status(HttpStatus.ACCEPTED).header("Cache-Control", "no-store").body(WebhookAdmissionResponse.from(outcome.delivery(), outcome.duplicate()));
    }

    @GetMapping("/api/v1/integrations/dlq")
    ResponseEntity<List<DeadLetterResponse>> deadLetters(@RequestParam(required=false) String connectorKey,@RequestParam(defaultValue="0") int offset,@RequestParam(defaultValue="50") int limit){
    ...
    DeadLetterResponse replay(@PathVariable String deliveryId,@RequestBody(required=false) ReasonRequest body,HttpServletRequest request){return DeadLetterResponse.from(operations.replay(DomainIdentifier.parse(deliveryId),actor(request),correlation(request),body==null?null:body.reason()));}

    @GetMapping("/api/v1/integrations/connectors/{connectorKey}/runtime")
    RuntimeStateResponse runtime(@PathVariable String connectorKey){return RuntimeStateResponse.from(operations.runtime(new ConnectorKey(connectorKey)), clock.instant());}

    @PostMapping("/api/v1/integrations/connectors/{connectorKey}/resume")
    RuntimeStateResponse resume(@PathVariable String connectorKey,@RequestBody(required=false) ReasonRequest body,HttpServletRequest request){return RuntimeStateResponse.from(operations.resume(new ConnectorKey(connectorKey), actor(request), correlation(request), body == null ? null : body.reason()), clock.instant());}
    ...
    private void validateJson(byte[] payload){try{JsonNode root=json.readTree(payload);if(root==null||(!root.isObject()&&!root.isArray()))throw new IllegalArgumentException("webhook payload root must be a JSON object or array");}catch(IOException invalid){throw new IllegalArgumentException("webhook payload is invalid JSON",invalid);}}
    private static long parseEpochSecond(String value){if(value==null||!value.matches("[0-9]{1,12}"))throw new IllegalArgumentException("webhook timestamp must be epoch seconds");return Long.parseLong(value);}
    private static DomainIdentifier actor(HttpServletRequest request){Object value=request.getAttribute(LocalAuthenticationFilter.ACCOUNT_ATTRIBUTE);if(value instanceof DomainIdentifier id)return id;throw new IllegalStateException("authenticated actor is missing");}
