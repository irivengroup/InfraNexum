...
public final class IdentityAccessController {
    ...
    @GetMapping("/api/v1/organizations/{orgId}/groups/{groupId}/members")
    ResponseEntity<List<GroupMemberResponse>> groupMembers(@PathVariable String orgId,@PathVariable String groupId,@RequestParam(defaultValue="0")int offset,@RequestParam(defaultValue="50")int limit){var page=service.groupMembers(id(orgId),id(groupId),offset,limit);return ApiPagination.offset(page.items().stream().map(GroupMemberResponse::from).toList(),page.nextOffset(),limit);}
