...
public final class ItamComplianceController {
    ...
    List<WarrantyTypeResponse> warrantyTypes(@RequestParam(name="organization_id") String organizationId,HttpServletRequest request,HttpServletResponse response){DomainIdentifier org=id(organizationId);authorization.require(request,response,PermissionCodes.ITAM_WARRANTY_READ,AuthorizationScope.organization(org),"itam-warranty-type","collection");return compliance.warrantyTypes().stream().map(WarrantyTypeResponse::from).toList();}
    ...
    List<AlertResponse> alerts(@PathVariable String assetId,@RequestParam(name="as_of",required=false) LocalDate asOf,@RequestParam(name="horizon_days",defaultValue="180") int horizon,HttpServletRequest request,HttpServletResponse response){Asset asset=requireAsset(assetId,PermissionCodes.ITAM_AUDIT_READ,request,response);return compliance.upcomingAlerts(asset.id(),asOf==null?LocalDate.now(clock):asOf,horizon).stream().map(AlertResponse::from).toList();}
