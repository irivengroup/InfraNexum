...
export function initializeTemporalPickers(documentObject = document) {
  ...
  const rangeController = initializeTemporalRanges(documentObject);
...
export function enhanceTemporalInput(documentObject, input) {
  ...
  const render = () => {
    ...
    const title = documentObject.createElement('button');
    title.type = 'button';
    title.className = 'inx-temporal-title';
    title.textContent = monthTitle(locale, view.year, view.month);
    title.setAttribute('aria-label', `${translate(locale, 'temporal.chooseYear')} ${view.year}`);
    title.addEventListener?.('click', () => { yearMode = !yearMode; render(); });
    ...
    for (const day of calendarMonthModel(view.year, view.month, draft.date)) {
      ...
      const outsideBounds = (input.min && day.iso < splitTemporalValue(input.min).date) || (input.max && day.iso > splitTemporalValue(input.max).date);
      cell.disabled = Boolean(outsideBounds);
      if (outsideBounds) cell.setAttribute('aria-disabled', 'true');
    ...
    if (yearMode) {
      const years = documentObject.createElement('div');
      years.className = 'inx-temporal-years';
      years.setAttribute('role', 'grid');
      const startYear = Math.floor(view.year / 12) * 12 - 1;
      for (let offset = 0; offset < 16; offset += 1) {
        const year = startYear + offset;
        const yearButton = documentObject.createElement('button');
        yearButton.type = 'button';
        yearButton.className = `inx-temporal-year${year === view.year ? ' active' : ''}`;
        yearButton.textContent = String(year);
        yearButton.setAttribute('aria-selected', year === view.year ? 'true' : 'false');
        yearButton.addEventListener?.('click', () => { view = { year, month: view.month }; yearMode = false; render(); });
        years.appendChild(yearButton);
      }
      const yearNav = documentObject.createElement('div');
      yearNav.className = 'inx-temporal-year-nav';
      const prevYears = button(documentObject, '−12', translate(locale, 'temporal.previousYears'));
      const nextYears = button(documentObject, '+12', translate(locale, 'temporal.nextYears'));
      prevYears.addEventListener?.('click', () => { view = { year: view.year - 12, month: view.month }; render(); });
      nextYears.addEventListener?.('click', () => { view = { year: view.year + 12, month: view.month }; render(); });
      yearNav.append(prevYears, nextYears);
      popover.append(header, years, yearNav, footer);
    } else {
  ...
  input.addEventListener?.('infranexum:temporal-sync', render);
  ...
  return Object.freeze({
    ...
    destroy() {
      input.removeEventListener?.('infranexum:temporal-sync', render);
...
/**
 * Enforces period coherence for all start/end temporal pairs in the same form.
 * End values may equal the start value, but can never precede it.
 */
export function initializeTemporalRanges(documentObject = document) {
  const pairs = [];
  const startNames = new Map([
    ['effectiveFrom', 'effectiveTo'], ['validFrom', 'validUntil'], ['startAt', 'endAt'], ['startsAt', 'endsAt'],
    ['startDate', 'endDate'], ['startsOn', 'endsOn'], ['warrantyStartDate', 'warrantyEndDate'], ['from', 'to'],
  ]);
  for (const form of documentObject?.querySelectorAll?.('form') ?? []) {
    for (const [startName, endName] of startNames) {
      const start = form.querySelector?.(`[name="${startName}"][data-inx-temporal]`);
      const end = form.querySelector?.(`[name="${endName}"][data-inx-temporal]`);
      if (!start || !end) continue;
      pairs.push(wireTemporalRange(start, end));
    }
  }
  return Object.freeze({ count: pairs.length, destroy() { pairs.forEach((pair) => pair.destroy()); } });
}

export function wireTemporalRange(start, end) {
  const validate = () => {
    const startValue = String(start.value ?? '').trim();
    const endValue = String(end.value ?? '').trim();
    end.min = startValue;
    start.max = endValue;
    if (startValue && endValue && temporalComparable(endValue) < temporalComparable(startValue)) {
      end.setCustomValidity?.('End date/time cannot be earlier than start date/time.');
      return false;
    }
    end.setCustomValidity?.('');
    return true;
  };
  const onStart = () => { validate(); end.dispatchEvent?.(customEvent(end, 'infranexum:temporal-sync')); };
  const onEnd = () => { validate(); };
  ...
  validate();
  return Object.freeze({ validate, destroy() {
    start.removeEventListener?.('input', onStart); start.removeEventListener?.('change', onStart);
    end.removeEventListener?.('input', onEnd); end.removeEventListener?.('change', onEnd);
  }});
}

function temporalComparable(value) {
  const normalized = String(value ?? '').trim();
  return normalized.length === 10 ? `${normalized}T00:00` : normalized;
}
function customEvent(input, type) {
  const EventConstructor = input?.ownerDocument?.defaultView?.Event ?? globalThis.Event;
  return typeof EventConstructor === 'function' ? new EventConstructor(type, { bubbles: false }) : { type };
}
