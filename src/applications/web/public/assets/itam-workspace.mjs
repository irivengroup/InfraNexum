...
export async function initializeItamWorkspace(documentObject = document, configuration, fetchFunction = fetch) {
  ...
  documentObject.addEventListener?.('infranexum:locale-change', () => rerender(documentObject, state));
...
export function itamWorkspaceTemplate(configuration = {}) {
  ...
  return `
    ...
    <div class="inx-domain-tabs" role="tablist" aria-label="ITAM">
      <button class="inx-domain-tab active" role="tab" type="button" data-itam-tab="partners" aria-selected="true" ${partnerEnabled ? '' : 'hidden'} data-i18n="itam.partners">Partners</button>
      <button class="inx-domain-tab" role="tab" type="button" data-itam-tab="assets" aria-selected="false" ${assetEnabled ? '' : 'hidden'} data-i18n="itam.assets">Assets</button>
      <button class="inx-domain-tab" role="tab" type="button" data-itam-tab="compliance" aria-selected="false" ${complianceEnabled ? '' : 'hidden'} data-i18n="itam.compliance">Compliance</button>
    </div>
    ${partnerPanel()}
    ${assetPanel()}
    ${compliancePanel()}`;
...
function partnerPanel() {
  return `<section class="inx-domain-panel" role="tabpanel" data-itam-panel="partners">
    <div class="inx-domain-split">
      <div>
        <form id="itam-partner-filter" class="inx-domain-toolbar row g-2">
        ...
        <pre id="itam-partner-detail" class="inx-domain-detail" tabindex="0">—</pre>
      </div>
      <div class="inx-domain-form-stack">
        <form id="itam-partner-create" class="inx-domain-form row g-2">
          ...
          <div class="col-md-4"><label class="form-label" for="itam-partner-country" data-i18n="itam.country">Country</label><input id="itam-partner-country" name="countryCode" class="form-control text-uppercase" minlength="2" maxlength="2" pattern="[A-Za-z]{2}" required /></div>
          <div class="col-md-8"><fieldset><legend class="form-label" data-i18n="itam.roles">Roles</legend><div class="inx-check-grid">${PARTNER_ROLES.map((r) => `<label><input type="checkbox" name="roles" value="${r}" /> <span data-i18n="itam.role.${r}">${r}</span></label>`).join('')}</div></fieldset></div>
          ...
          ${jsonField('itam-partner-contacts','contacts','itam.contacts','[]',4)}
...
function assetPanel() {
  return `<section class="inx-domain-panel" role="tabpanel" data-itam-panel="assets" hidden aria-hidden="true"><div class="inx-domain-split"><div>
    ...
    <pre id="itam-asset-detail" class="inx-domain-detail" tabindex="0">—</pre>
    <h3 class="inx-domain-subtitle" data-i18n="itam.custody">Custody chain</h3>${table('itam-custody-table-body', [['itam.sequence','Sequence'],['itam.event','Event'],['common.status','Status'],['itam.custodian','Custodian'],['common.reason','Reason'],['common.updated','Occurred']])}
  </div><div class="inx-domain-form-stack">
    <form id="itam-asset-create" class="inx-domain-form row g-2"><h3 data-i18n="itam.assetCreate">Acquire asset</h3>
    ...
    <form id="itam-asset-lifecycle" class="inx-domain-form row g-2"><h3 data-i18n="itam.assetLifecycle">Selected asset lifecycle</h3>
...
function compliancePanel() {
  return `<section class="inx-domain-panel" role="tabpanel" data-itam-panel="compliance" hidden aria-hidden="true">
    <div class="inx-domain-context row g-2"><div class="col-lg-8"><label class="form-label" for="itam-compliance-asset" data-i18n="itam.asset">Asset</label><select id="itam-compliance-asset" class="form-select"></select></div><div class="col-lg-4 d-flex align-items-end"><button id="itam-compliance-refresh" class="btn btn-outline-primary w-100" type="button" data-i18n="common.refresh">Refresh</button></div></div>
    <div class="inx-domain-tabs inx-domain-subtabs" role="tablist"><button class="inx-domain-tab active" role="tab" type="button" data-compliance-tab="warranties" aria-selected="true" data-i18n="itam.warranties">Warranties</button><button class="inx-domain-tab" role="tab" type="button" data-compliance-tab="licenses" aria-selected="false" data-i18n="itam.licenses">Licenses</button><button class="inx-domain-tab" role="tab" type="button" data-compliance-tab="coverage" aria-selected="false" data-i18n="itam.supportCoverage">Support coverage</button><button class="inx-domain-tab" role="tab" type="button" data-compliance-tab="catalog" aria-selected="false" data-i18n="itam.supportCatalog">Support catalog</button><button class="inx-domain-tab" role="tab" type="button" data-compliance-tab="alerts" aria-selected="false" data-i18n="itam.alerts">Alerts / history</button></div>
    ${warrantyPanel()}${licensePanel()}${coveragePanel()}${catalogPanel()}${alertPanel()}
  </section>`;
}

function warrantyPanel() { return `<section class="inx-domain-subpanel" data-compliance-panel="warranties"><div class="inx-domain-split"><div>${table('itam-warranty-table-body', [['rsot.id','ID'],['itam.manufacturer','Manufacturer'],['itam.warrantyEnd','Warranty end'],['itam.supportEnd','Support end'],['common.status','Status'],['common.version','Version']])}<pre id="itam-warranty-detail" class="inx-domain-detail">—</pre></div><form id="itam-warranty-form" class="inx-domain-form row g-2"><h3 data-i18n="itam.warrantyManage">Warranty</h3><input name="recordId" type="hidden" /><input name="version" type="hidden" />
...
function licensePanel() { return `<section class="inx-domain-subpanel" data-compliance-panel="licenses" hidden aria-hidden="true"><div class="inx-domain-split"><div>${table('itam-license-table-body', [['rsot.id','ID'],['itam.publisher','Publisher'],['itam.contractNumber','Contract'],['itam.supportEnd','Support end'],['common.status','Status'],['common.version','Version']])}<pre id="itam-license-detail" class="inx-domain-detail">—</pre></div><form id="itam-license-form" class="inx-domain-form row g-2"><h3 data-i18n="itam.licenseManage">Software license</h3><input name="recordId" type="hidden" /><input name="version" type="hidden" />
  ...
  <div class="col-12 inx-security-note" data-i18n="itam.licenseSecretNotice">License/product keys are intentionally not accepted until Secret Service/PKI/KMS is available.</div><div class="col-12 d-flex gap-2 flex-wrap"><button class="btn btn-primary" type="submit" value="save" data-i18n="common.save">Save</button><button class="btn btn-outline-success" type="submit" value="activate" data-i18n="common.activate">Activate</button><button class="btn btn-outline-warning" type="submit" value="expire" data-i18n="itam.expire">Expire</button><button class="btn btn-outline-secondary" type="button" id="itam-license-clear" data-i18n="common.clear">Clear selection</button></div></form></div></section>`; }

function coveragePanel() { return `<section class="inx-domain-subpanel" data-compliance-panel="coverage" hidden aria-hidden="true"><div class="inx-domain-split"><div>${table('itam-coverage-table-body', [['rsot.id','ID'],['itam.supportProvider','Provider'],['itam.serviceLevel','Service level'],['common.validUntil','Ends'],['common.status','Status'],['common.version','Version']])}<pre id="itam-coverage-detail" class="inx-domain-detail">—</pre></div><form id="itam-coverage-form" class="inx-domain-form row g-2"><h3 data-i18n="itam.coverageManage">Support coverage</h3><input name="recordId" type="hidden" /><input name="version" type="hidden" />
...
async function loadDirectories(documentObject, configuration, fetchFunction, state) {
  ...
  fillSelect(documentObject, documentObject.getElementById('itam-organization'), state.organizations, { label: (x) => `${x.code ?? ''} — ${x.displayName ?? x.id}`, preserve: false });
  const select = documentObject.getElementById('itam-organization'); if (select && !select.value && state.organizations[0]) select.value = state.organizations[0].id;
}
...
function wirePartner(documentObject, client, state) {
  if (!client) return;
  documentObject.getElementById('itam-partner-filter')?.addEventListener('submit', (event) => { event.preventDefault(); void loadPartners(documentObject, client, state); });
...
function renderPartners(documentObject,state){replaceRows(documentObject,documentObject.getElementById('itam-partner-table-body'),state.partners,[(x)=>x.code,(x)=>x.displayName,(x)=>(x.roles??[]).join(', '),(x)=>x.authorizationStatus,(x)=>x.version],(item)=>{state.selectedPartner=item;documentObject.getElementById('itam-partner-detail').textContent=JSON.stringify(item,null,2);});}
...
function wireAsset(documentObject, configuration, fetchFunction, clients, state) {
  ...
  documentObject.getElementById('itam-asset-filter')?.addEventListener('submit',(event)=>{event.preventDefault();void loadAssets(documentObject,client,state);});
...
function wireCompliance(documentObject,client,state){if(!client)return;documentObject.getElementById('itam-compliance-asset')?.addEventListener('change',()=>void refreshComplianceRecords(documentObject,client,state));documentObject.getElementById('itam-compliance-refresh')?.addEventListener('click',()=>void refreshComplianceRecords(documentObject,client,state));
  ...
  documentObject.getElementById('itam-alert-filter')?.addEventListener('submit',(e)=>{e.preventDefault();void loadAlerts(documentObject,client,state);});documentObject.getElementById('itam-history-filter')?.addEventListener('submit',(e)=>{e.preventDefault();void loadHistory(documentObject,client,state);});documentObject.getElementById('itam-history-type')?.addEventListener('change',()=>refillHistoryRecords(documentObject,state));
}
...
function table(tbodyId,headings){return `<div class="inx-table-shell inx-domain-table"><div class="table-responsive"><table class="table inx-data-table"><thead><tr>${headings.map(([k,f])=>`<th scope="col" data-i18n="${k}">${f}</th>`).join('')}</tr></thead><tbody id="${tbodyId}"></tbody></table></div></div>`;}
