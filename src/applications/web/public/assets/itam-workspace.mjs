...
export async function initializeItamWorkspace(documentObject = document, configuration, fetchFunction = fetch) {
  ...
  const state = {
    ...
    selectedPartner: null, selectedAsset: null, selectedCompliance: null, selectedAuthorization: null, selectedWarrantyType: null,
  };
  ...
  prepareItamCrudPanels(documentObject, state);
  applyTranslations(documentObject, localeFromDocument(documentObject));
...
function partnerPanel() {
  return `<section class="tab-pane" role="tabpanel" data-itam-panel="partners">
    <div class="d-grid gap-3">
      <div>
        <form id="itam-partner-filter" class="inx-filter-bar mb-4">
          <div class="inx-filter-field inx-filter-field-wide"><label class="form-label" for="itam-partner-role-filter" data-i18n="itam.role">Role</label><select id="itam-partner-role-filter" name="role" class="form-select"><option value="" data-i18n="common.all">All</option>${PARTNER_ROLES.map((r) => `<option value="${r}" data-i18n="itam.role.${r}">${r}</option>`).join('')}</select></div>
      ...
      <div class="d-grid gap-3">
        <form id="itam-partner-create" class="border rounded-3 p-3 p-xl-4 bg-body-tertiary row g-3">
          ...
          <div class="col-12"><label class="form-label" for="itam-partner-display" data-i18n="itam.displayName">Display name</label><input id="itam-partner-display" name="displayName" class="form-control" maxlength="255" required /></div>
...
function compliancePanel() {
  return `<section class="tab-pane" role="tabpanel" data-itam-panel="compliance" hidden aria-hidden="true">
    ...
    <div class="nav nav-underline gap-3 mb-4 border-bottom" role="tablist"><button class="nav-link active" role="tab" type="button" data-compliance-tab="warranties" aria-selected="true" data-i18n="itam.warranties">Warranties</button><button class="nav-link" role="tab" type="button" data-compliance-tab="licenses" aria-selected="false" data-i18n="itam.licenses">Licenses</button><button class="nav-link" role="tab" type="button" data-compliance-tab="coverage" aria-selected="false" data-i18n="itam.supportCoverage">Support coverage</button><button class="nav-link" role="tab" type="button" data-compliance-tab="authorizations" aria-selected="false" data-i18n="itam.supportAuthorizations">Support authorizations</button><button class="nav-link" role="tab" type="button" data-compliance-tab="warranty-types" aria-selected="false" data-i18n="itam.warrantyTypes">Warranty types</button><button class="nav-link" role="tab" type="button" data-compliance-tab="alerts" aria-selected="false" data-i18n="itam.alerts">Alerts</button><button class="nav-link" role="tab" type="button" data-compliance-tab="history" aria-selected="false" data-i18n="itam.history">History</button></div>
    ${warrantyPanel()}${licensePanel()}${coveragePanel()}${authorizationPanel()}${warrantyTypePanel()}${alertPanel()}${historyPanel()}
  </section>`;
}
...
function prepareItamCrudPanels(documentObject, state) {
  const specs = [
    { selector: '[data-itam-panel="partners"]', key: 'itam-partners', forms: [['itam-partner-create','create','itam.partnerCreate'],['itam-partner-lifecycle','lifecycle','itam.partnerLifecycle']], newKey: 'create', newTitle: 'itam.partnerCreate' },
    { selector: '[data-itam-panel="assets"]', key: 'itam-assets', forms: [['itam-asset-create','create','itam.assetCreate'],['itam-asset-lifecycle','lifecycle','itam.assetLifecycle']], newKey: 'create', newTitle: 'itam.assetCreate' },
    { selector: '[data-compliance-panel="warranties"]', key: 'itam-warranties', forms: [['itam-warranty-form','editor','itam.warranties']], newKey: 'editor', newTitle: 'itam.warranties', reset: () => clearRecordSelection(documentObject,state,'warranty') },
    { selector: '[data-compliance-panel="licenses"]', key: 'itam-licenses', forms: [['itam-license-form','editor','itam.licenses']], newKey: 'editor', newTitle: 'itam.licenses', reset: () => clearRecordSelection(documentObject,state,'license') },
    { selector: '[data-compliance-panel="coverage"]', key: 'itam-coverage', forms: [['itam-coverage-form','editor','itam.supportCoverage']], newKey: 'editor', newTitle: 'itam.supportCoverage', reset: () => clearRecordSelection(documentObject,state,'coverage') },
    { selector: '[data-compliance-panel="authorizations"]', key: 'itam-authorizations', forms: [['itam-authorization-create','create','itam.authorizationCreate'],['itam-authorization-lifecycle','lifecycle','itam.supportAuthorizations']], newKey: 'create', newTitle: 'itam.authorizationCreate' },
    { selector: '[data-compliance-panel="warranty-types"]', key: 'itam-warranty-types', forms: [['itam-warranty-type-create','create','itam.warrantyTypeCreate']], newKey: 'create', newTitle: 'itam.warrantyTypeCreate' },
  ];
  for (const spec of specs) configureItamCrudPanel(documentObject, spec);
  initializeEnterpriseDataTables(documentObject);
}

function configureItamCrudPanel(documentObject, spec) {
  const panel = documentObject.querySelector?.(spec.selector);
  if (!panel || panel.getAttribute?.('data-inx-crud-panel')) return;
  const forms = spec.forms.map(([id,key,title]) => ({ form: documentObject.getElementById(id), key, title })).filter((entry) => entry.form);
  if (forms.length === 0) return;
  for (const { form } of forms) form.remove?.();
  const list = documentObject.createElement('div');
  list.className = 'inx-crud-list-view'; list.setAttribute('data-inx-crud-list','');
  while (panel.firstElementChild) list.appendChild(panel.firstElementChild);
  const toolbar = documentObject.createElement('div'); toolbar.className = 'inx-crud-toolbar';
  if (spec.newKey) toolbar.innerHTML = `<button class="btn btn-primary btn-sm" type="button" data-inx-crud-new="${spec.newKey}" data-inx-crud-editor-mode="create"><span aria-hidden="true">＋</span> <span data-i18n="common.new">New</span></button>`;
  if (toolbar.childNodes?.length) list.insertBefore(toolbar, list.firstChild);
  const editor = documentObject.createElement('section'); editor.className = 'inx-crud-editor-view'; editor.setAttribute('data-inx-crud-editor',''); editor.hidden = true; editor.setAttribute('aria-hidden','true');
  const head = documentObject.createElement('div'); head.className='inx-crud-editor-header'; head.innerHTML=`<h3 class="h5 mb-0" data-inx-crud-editor-title>—</h3><button class="btn btn-outline-secondary btn-sm" type="button" data-inx-crud-back data-i18n="common.backToList">Back to list</button>`; editor.appendChild(head);
  for (const { form,key,title } of forms) { form.setAttribute('data-inx-crud-form',key); form.setAttribute('data-inx-crud-title-key',title); editor.appendChild(form); }
  panel.classList?.add?.('inx-crud-panel'); panel.setAttribute('data-inx-crud-panel',spec.key); panel.replaceChildren(list,editor);
  wireCrudPanel(documentObject,panel);
  const createButton = panel.querySelector?.('[data-inx-crud-new]');
  if (createButton && typeof spec.reset === 'function') createButton.addEventListener?.('click', () => spec.reset());
}
...
function renderPartners(documentObject,state){const panel=documentObject.querySelector?.('[data-itam-panel="partners"]');replaceRows(documentObject,documentObject.getElementById('itam-partner-table-body'),state.partners,[(x)=>x.code,(x)=>x.displayName,(x)=>(x.roles??[]).join(', '),(x)=>x.authorizationStatus,(x)=>x.version],(item)=>{state.selectedPartner=item;renderPartnerDetail(documentObject,item);},{actions:()=>[{key:'lifecycle',labelKey:'itam.partnerLifecycle',className:'btn-outline-primary',onClick:()=>openCrudEditor(panel,'lifecycle',{mode:'edit'})}]});initializeEnterpriseDataTables(documentObject);}
...
function renderAssets(documentObject,state){const panel=documentObject.querySelector?.('[data-itam-panel="assets"]');replaceRows(documentObject,documentObject.getElementById('itam-asset-table-body'),state.assets,[(x)=>x.id,(x)=>x.assetType,(x)=>x.rsotObjectId,(x)=>x.lifecycleStatus,(x)=>partnerLabel(state,x.producerPartnerId),(x)=>x.version],(item)=>void selectAsset(documentObject,state,item),{actions:()=>[{key:'lifecycle',labelKey:'itam.assetLifecycle',className:'btn-outline-primary',onClick:()=>openCrudEditor(panel,'lifecycle',{mode:'edit'})}]});initializeEnterpriseDataTables(documentObject);}
...
function wireCompliance(documentObject,client,state){if(!client)return;documentObject.getElementById('itam-compliance-asset')?.addEventListener('change',()=>void refreshComplianceRecords(documentObject,client,state));documentObject.getElementById('itam-compliance-refresh')?.addEventListener('click',()=>void refreshComplianceRecords(documentObject,client,state));
  ...
  wireAsyncForm(documentObject.getElementById('itam-warranty-type-create'),{execute:async(form)=>{await client.createWarrantyType(contextOrganization(documentObject),field(form,'displayName'),field(form,'reason'),idempotencyKey('warranty-type'));await loadComplianceCatalogs(documentObject,client,state);refillEntitySelectors(documentObject,state);},onWorking:()=>working(documentObject),onSuccess:()=>saved(documentObject),onError:(e)=>showError(documentObject,e)});
...
function renderComplianceCatalogs(documentObject,state){const authorizationPanel=documentObject.querySelector?.('[data-compliance-panel="authorizations"]');replaceRows(documentObject,documentObject.getElementById('itam-authorization-table-body'),state.authorizations,[(x)=>x.id,(x)=>partnerLabel(state,x.providerPartnerId),(x)=>x.status,(x)=>x.validFrom,(x)=>x.validUntil,(x)=>x.version],(item)=>{state.selectedAuthorization=item;},{actions:()=>[{key:'lifecycle',labelKey:'common.actions',className:'btn-outline-primary',onClick:()=>openCrudEditor(authorizationPanel,'lifecycle',{mode:'edit'})}]});replaceRows(documentObject,documentObject.getElementById('itam-warranty-type-table-body'),state.warrantyTypes,[(x)=>x.id,(x)=>x.code,(x)=>x.displayName,(x)=>x.active?'active':'inactive'],(item)=>{state.selectedWarrantyType=item;},{actions:()=>[]});initializeEnterpriseDataTables(documentObject);}
...
function renderComplianceRows(documentObject,state,warranties=state.warranties??[],licenses=state.licenses??[],coverages=state.coverages??[]){for(const [kind,rows,target,cells] of [['warranty',warranties,'itam-warranty-table-body',[(x)=>x.id,(x)=>partnerLabel(state,x.manufacturerPartnerId),(x)=>x.warrantyEndDate,(x)=>x.manufacturerSupportEndDate,(x)=>x.status,(x)=>x.version]],['license',licenses,'itam-license-table-body',[(x)=>x.id,(x)=>partnerLabel(state,x.publisherPartnerId),(x)=>x.contractNumber,(x)=>x.publisherSupportEndDate,(x)=>x.status,(x)=>x.version]],['coverage',coverages,'itam-coverage-table-body',[(x)=>x.id,(x)=>partnerLabel(state,x.providerPartnerId),(x)=>x.serviceLevel,(x)=>x.endsOn,(x)=>x.status,(x)=>x.version]]]){const panel=documentObject.querySelector?.(`[data-compliance-panel="${kind==='warranty'?'warranties':kind==='license'?'licenses':'coverage'}"]`);replaceRows(documentObject,documentObject.getElementById(target),rows,cells,(x)=>selectCompliance(documentObject,state,kind,x),{actions:()=>[{key:'edit',labelKey:'common.edit',className:'btn-outline-primary',onClick:()=>openCrudEditor(panel,'editor',{mode:'edit'})}]});}initializeEnterpriseDataTables(documentObject);}
...
async function loadAlerts(documentObject,client,state){const assetId=complianceAssetId(documentObject);if(!assetId)return;const form=documentObject.getElementById('itam-alert-filter');const result=await client.alerts(assetId,{asOf:nullable(field(form,'asOf')),horizonDays:Number(field(form,'horizonDays')||180)});state.alerts=listItems(result.payload);replaceRows(documentObject,documentObject.getElementById('itam-alert-table-body'),state.alerts,[(x)=>x.kind,(x)=>x.recordId,(x)=>x.dueDate,(x)=>x.daysRemaining,(x)=>x.thresholdDays],()=>{}, {actions:(item)=>[{key:'open',labelKey:'common.open',className:'btn-outline-primary',onClick:()=>renderReadonlyDetail(documentObject,'itam-alert-detail',[[translate(localeFromDocument(documentObject),'itam.alertKind'),item.kind],[translate(localeFromDocument(documentObject),'itam.record'),item.recordId],[translate(localeFromDocument(documentObject),'itam.asset'),item.assetId],[translate(localeFromDocument(documentObject),'itam.dueDate'),item.dueDate],[translate(localeFromDocument(documentObject),'itam.daysRemaining'),item.daysRemaining],[translate(localeFromDocument(documentObject),'itam.threshold'),item.thresholdDays]])}]});initializeEnterpriseDataTables(documentObject);}
async function loadHistory(documentObject,client,state){const form=documentObject.getElementById('itam-history-filter');const type=field(form,'type'),recordId=field(form,'recordId');const detail=documentObject.getElementById('itam-history-detail');if(!recordId){state.history=[];replaceRows(documentObject,documentObject.getElementById('itam-history-table-body'),[],[(x)=>x.recordType,(x)=>x.recordId,(x)=>x.version,(x)=>x.status,(x)=>x.proofReference,(x)=>x.recordedAt],()=>{}, {actions:()=>[]});if(detail){detail.hidden=true;detail.replaceChildren();}return;}const result=await client.history(type,recordId,{limit:200});state.history=listItems(result.payload);replaceRows(documentObject,documentObject.getElementById('itam-history-table-body'),state.history,[(x)=>x.recordType,(x)=>x.recordId,(x)=>x.version,(x)=>x.status,(x)=>x.proofReference??'—',(x)=>x.recordedAt],()=>{}, {actions:(item)=>[{key:'open',labelKey:'common.open',className:'btn-outline-primary',onClick:()=>renderReadonlyDetail(documentObject,'itam-history-detail',[[translate(localeFromDocument(documentObject),'itam.recordType'),item.recordType],[translate(localeFromDocument(documentObject),'itam.record'),item.recordId],[translate(localeFromDocument(documentObject),'common.version'),item.version],[translate(localeFromDocument(documentObject),'common.status'),item.status],[translate(localeFromDocument(documentObject),'itam.proof'),item.proofReference??'—'],[translate(localeFromDocument(documentObject),'common.reason'),item.reason],[translate(localeFromDocument(documentObject),'common.updated'),item.recordedAt]])}]});initializeEnterpriseDataTables(documentObject);}

function renderReadonlyDetail(documentObject,targetId,entries){const host=documentObject.getElementById(targetId);if(!host)return;const body=documentObject.createElement('div');body.className='card-body p-3 p-xl-4';const grid=documentObject.createElement('dl');grid.className='row g-3 mb-0 inx-record-detail-grid';for(const [label,value] of entries){const wrap=documentObject.createElement('div');wrap.className='col-sm-6 col-xl-4';const term=documentObject.createElement('dt');term.className='small text-uppercase text-body-secondary mb-1';term.textContent=String(label??'');const definition=documentObject.createElement('dd');definition.className='mb-0 fw-semibold text-break';definition.textContent=value===null||value===undefined||value===''?'—':String(value);wrap.append(term,definition);grid.appendChild(wrap);}body.appendChild(grid);host.replaceChildren(body);host.hidden=false;host.focus?.();}
