...
export async function initializeDcimWorkspace(documentObject = document, configuration, fetchFunction = fetch) {
  ...
  try {
    ...
    fillSelect(documentObject,documentObject.getElementById('dcim-organization'),state.organizations,{label:(item)=>`${item.code??''} — ${item.displayName??item.legalName??item.id}`});
...
function bindResource(d,c,s,resource){
  ...
  form?.addEventListener('submit',(event)=>{event.preventDefault?.();void mutateCreate(d,c,s,resource,form);});
  d.getElementById(`dcim-${resource}-update`)?.addEventListener('click',()=>void mutateUpdate(d,c,s,resource,form));
  d.getElementById(`dcim-${resource}-clear`)?.addEventListener('click',()=>clearSelection(d,s,resource));
  const statusForm=d.getElementById(`dcim-${resource}-status-form`);statusForm?.addEventListener('submit',(event)=>{event.preventDefault?.();void mutateStatus(d,c,s,resource,statusForm);});
}
