...
POLICY = SOURCE_ROOT / "src/validation/architecture/policy.json"
...
class ArchitectureCheckerTest(unittest.TestCase):
    ...
    def run_check(self):
        return ArchitectureChecker(self.root, self.root / "src/validation/architecture/policy.json").run()
    ...
    def test_clean_generated_target_removes_artifacts_portably(self) -> None:
        ...
        cache = self.root / "src/validation" / "__pycache__"
    ...
    def test_source_files_outside_src_are_blocked(self) -> None:
    ...
    def test_source_root_policy_is_validated(self) -> None:
        policy_path = self.root / "src/validation/architecture/policy.json"
    ...
    def test_language_and_namespace_errors_are_blocked(self) -> None:
        bad_go = self.root / "src/applications/server/src/bad.go"
        bad_go.parent.mkdir(parents=True, exist_ok=True)
        bad_go.write_text("package bad", encoding="utf-8")
        bad_java = self.root / "src/components/core/contracts/src/main/java/Bad.java"
    ...
    def test_active_component_without_source_is_blocked(self) -> None:
        source = self.root / "src/components/core/contracts/src"
    ...
    def test_external_violation_path_is_rendered_safely(self) -> None:
        checker = ArchitectureChecker(self.root, self.root / "src/validation/architecture/policy.json")
    ...
    def test_report_is_deterministic_and_serializable(self) -> None:
        ...
        self.assertEqual("src/validation/architecture/policy.json", payload["policy"])
