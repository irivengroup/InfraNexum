# InfraNexum 2.0.0-alpha.0.68 — PGM-03-E03 RBAC Foundation

**InfraNexum — Infrastructure Control & Governance Platform**
`alpha.0.68` introduces the PGM-03-E03 identity-access bounded context: IAM users, organization memberships, groups, protected roles, the approved atomic permission catalogue and temporal scoped assignments. Authorization is deny-by-default at the Server HTTP boundary, uses the same RBAC evaluator from API and Server-owned CLI paths, and writes append-only audit evidence for every evaluated decision. Existing local-account UUIDs are preserved as IAM user identifiers and upgrade/bootstrap accounts receive the protected global `system.platform_admin` role so enabling enforcement cannot lock out an existing installation.

PostgreSQL and Oracle migration `0013-identity-access-rbac-foundation` are paired and additive. The Web administration surface exposes users, groups, roles and permissions only when local authentication and the organization foundation are enabled, with CSRF-protected mutations and DE/EN/ES/FR/IT labels. Full ABAC/PDP/SoD remains intentionally outside this increment and belongs to PGM-03-E04. The draft.21 catalogue contains no `organization.read` permission; organization-root reads therefore use effective IAM membership or `system.platform_admin` rather than inventing a non-normative permission.
