# InfraNexum 2.0.0-alpha.0.57 — Organization & Subdivision Foundation

**InfraNexum — Infrastructure Control & Governance Platform**

`alpha.0.57` opens the first authoritative business bounded context after the Core/Persistence foundation: Organization, Subdivision and effective-dated governance scopes. Organizations use UUIDv7 identities, globally unique normalized codes, optimistic versions and an explicit lifecycle. Subdivisions are organization-scoped, use organization-local unique codes, enforce same-organization parent references and bounded hierarchy depth, and remain unavailable when the active profile does not support them. Temporal scopes are typed and half-open. Creation and lifecycle mutations are idempotent and append transactional outbox events using Core's canonical versioned event-type contract.
