...
Provider identity remains immutable: a tombstone target cannot reuse the configured identity field/attribute. `IGNORE` rejects tombstone configuration, `TOMBSTONE` requires it, `MANUAL` never enables automatic propagation, and an absent remote object is never created merely to tombstone it. No physical provider `DELETE` is emitted. The JDBC outbound source reads `lifecycle_status` solely to derive the internal deletion marker and does not expose that field unless it is itself governed.

The Integrations Web workspace also fixes a functional governance defect: Execute previously submitted an empty `fields` list. It now derives the exact direction and field set from the active policy, locks direction to that policy, and exposes a deletion-propagation checkbox disabled by default and enabled only for `TOMBSTONE`. The generic connector-sync API remains the sole mutation entrypoint; no public API operation or database migration is added.

**Qualification status:** Source Integrity, Architecture-as-Code, affected architecture/Compose contracts, JDK 25 tombstone/JDBC targeted tests, API checker and the Node 24 Web suite are executed locally. The full Server/JDBC Maven + JaCoCo reactor and PostgreSQL 17/18 live matrix still require the complete dependency/database environment; those gates remain mandatory and are not counted as passed.
...
`alpha.0.127` is a Web/UX corrective over the vendored ReDoc 2.5.3 / Node 24.19.0 baseline. It does not add an API operation, migration, RBAC permission, capability or mutating connector provider.

- **ReDoc:** the lateral menu has natural expandable/collapsible height; the document column keeps only the active API section visible; the iframe follows intrinsic height upward and downward after startup, without a viewport minimum ratchet, nested vertical scrolling or active-text truncation.
- **DCIM:** Location CRUD panels are explicitly wired to the shared enterprise CRUD controller, restoring `+ New`; the contextual navigation uses the same visual contract as Identity & Access.
- **Integrations:** Governance, Synchronization, Jira Assets, ServiceNow and Notifications are presented as vertical tabs on the IAM/DCIM model while preserving their existing forms, IDs and provider behavior.
- **Source integrity:** `business-form-focus.mjs` remains intentionally absent following the alpha.0.124 root-cause correction. Any stale tracked/staged copy in an older checkout must be removed, not added to the canonical inventory.

Validation evidence and remaining qualification gates are recorded in `docs/implementation-status.md` and `src/distribution/release-manifest.json`. PGM-10-E05 remains formally **NON TERMINÉ** pending the exact JDK25/JaCoCo/PostgreSQL 17/18 gates; PGM-10-E06 remains **EN COURS** with no mutating provider activated.
...
`alpha.0.123` corrects the authenticated business-form focus boundary without changing login or search behavior. Editable CRUD inputs now reclaim pointer focus from the route container before typing, preserving native caret/selection semantics and avoiding keyboard interception. The regression is covered both by Web tests and by a real Chromium CDP scenario that starts with `<main>` focused, clicks a business input and verifies that the typed character is written to that input.

DCIM Infrastructure now exposes a governed equipment taxonomy with **15 categories / 97 equipment types**, category-filtered type selection and explicit rack-mountability. Equipment models carry manufacturer references and repeatable heterogeneous port templates; installed equipment inherits its model metadata. Cabling now records Equipment A / Port A and Equipment B / Port B using existing selectable objects, filters endpoints to ports actually owned by the selected equipment, and records cable type, length and manufacturer reference. Paired migration `0040` adds the taxonomy/cable metadata for PostgreSQL and Oracle while preserving existing rows and refusing lossy rollback when new metadata is in use.

Business catalogue creation no longer requires operators to invent technical codes where a business name already exists. DCIM facilities/models/racks, DDI VRFs, ITAM partners/warranty types and RSOT schema profiles generate memorable codes inside their bounded-context services; legacy API/CLI clients may still supply compatible codes. Each domain preserves its existing syntax invariant (for example lowercase RSOT profile codes and underscore-based warranty type codes). IAM permission/policy codes remain explicit because they are security contract identifiers rather than display-name-derived catalogue codes.

**Validation executed locally:** Web **228/228** with **99.73% lines / 98.56% branches / 100% functions** and process smoke passed; Chromium CDP focus/typing regression passed; API contracts **48/48**, **15 fragments / 201 operations**, debt **0/0/0/0**; migrations **118/118** with zero violations; Toolchains **25/25** with zero violations; Compose contracts **69/69**; six affected Java smoke suites passed under the available Java 21 preflight; architecture functional tests **207/207** and Architecture-as-Code reports zero violations.

**Roadmap status:** PGM-10-E05 remains formally **NON TERMINÉ** until the exact JDK25/Maven/JUnit/JaCoCo and PostgreSQL 17/18 gates execute successfully on this source snapshot. PGM-10-E06 remains **EN COURS**; durable checkpoints/synchronization/compensation and prepared authority admission are preserved, and no Jira Assets or ServiceNow mutator is activated.
