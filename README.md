# InfraNexum 2.0.0-alpha.0.58 — Organization & Subdivision Foundation

**InfraNexum — Infrastructure Control & Governance Platform**

`alpha.0.58` turns the Web foundation into a professional administration dashboard while preserving the authoritative Organization/Subdivision foundation introduced previously. The responsive application shell uses vendored Bootstrap 5.3.6 plus the adapted InfraNexum visual theme, exposes only truthful runtime/organization state, provides an accessible persisted light/dark preference, and keeps all API access same-origin through the Web HA router. Organizations use UUIDv7 identities, globally unique normalized codes, optimistic versions and an explicit lifecycle. Subdivisions are organization-scoped, use organization-local unique codes, enforce same-organization parent references and bounded hierarchy depth, and remain unavailable when the active profile does not support them. Temporal scopes are typed and half-open. Creation and lifecycle mutations are idempotent and append transactional outbox events using Core's canonical versioned event-type contract.

Persistence is implemented for PostgreSQL and Oracle through paired migration `0010-organization-foundation`, with database-level UUIDv7, lifecycle, parent-scope and idempotency constraints plus rollback and verification contracts. The HTTP adapter is intentionally pre-IAM: it is disabled by default and refuses to initialize outside `environment=local`. Docker development explicitly enables it only in the local PRO topology. Browser API calls stay same-origin through the Web HAProxy `/api` route instead of enabling cross-origin access.

The Web foundation now lists organizations and allows an operator to inspect subdivisions of the selected organization. It uses locally vendored Bootstrap 5.3.6 followed by the adapted predecessor visual theme only; business values are rendered with DOM `textContent`, never injected as HTML. The production deployment boundary is unchanged: standalone bare metal or VM; Docker/Compose remains development and test tooling only.

The preceding `alpha.0.56` PRO HA runtime is now validated end-to-end on Docker Desktop: nominal smoke passed with three PostgreSQL/Patroni nodes, four Server nodes and two Web nodes; PostgreSQL primary failover/rejoin, Server-node failover and Web-node failover all passed; final Server readiness returned `UP`. This provides the runtime foundation on which the Organization vertical slice is built.
