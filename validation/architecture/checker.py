...
class ArchitectureChecker:
    """Validates repository structure, manifests, ownership, and language boundaries."""

    _CODE_EXTENSIONS = frozenset({
        ".java", ".go", ".ts", ".tsx", ".js", ".jsx", ".mjs", ".cjs",
        ".py", ".cc", ".cpp", ".cxx", ".h", ".hpp", ".sh", ".ps1",
    })
    ...
    def _check_root(self) -> None:
        for name in self.policy["required_structural_spaces"]:
            path = self.root / name
            if not path.is_dir():
                self._add("CHECK-ARCH-ROOT-001", path, f"required structural space {name!r} is missing")
    ...
    def _load_and_validate_manifests(self) -> None:
        ...
        for relative in self.policy["required_manifest_paths"]:
            manifest_path = self.root / relative / "MANIFEST.json"
            if not manifest_path.is_file():
                self._add("CHECK-ARCH-MANIFEST-001", manifest_path, "required component manifest is missing")
                continue
            try:
                payload = self._load_json(manifest_path)
            except (OSError, ValueError) as error:
                self._add("CHECK-ARCH-MANIFEST-002", manifest_path, f"invalid JSON manifest: {error}")
                continue
            required = {"schema", "id", "kind", "owner", "lifecycle", "languages", "dependencies", "source_baseline"}
            missing = sorted(required - payload.keys())
            if missing:
                self._add("CHECK-ARCH-MANIFEST-003", manifest_path, f"missing fields: {missing}")
                continue
            if payload["schema"] != expected_schema:
                self._add("CHECK-ARCH-MANIFEST-004", manifest_path, f"schema must be {expected_schema!r}")
            expected_id = relative.replace("/", ".")
            if payload["id"] != expected_id:
                self._add("CHECK-ARCH-MANIFEST-005", manifest_path, f"id must be {expected_id!r}")
            if payload["owner"] not in owners:
                self._add("CHECK-ARCH-OWNER-001", manifest_path, f"unknown owner {payload['owner']!r}")
            if payload["lifecycle"] not in {"active", "planned", "deprecated"}:
                self._add("CHECK-ARCH-MANIFEST-006", manifest_path, "lifecycle must be active, planned, or deprecated")
            if not isinstance(payload["languages"], list) or not all(isinstance(item, str) for item in payload["languages"]):
                self._add("CHECK-ARCH-MANIFEST-007", manifest_path, "languages must be a string array")
                continue
            if not isinstance(payload["dependencies"], list) or not all(isinstance(item, str) for item in payload["dependencies"]):
                self._add("CHECK-ARCH-MANIFEST-008", manifest_path, "dependencies must be a string array")
                continue
            if not isinstance(payload["source_baseline"], list) or not payload["source_baseline"]:
                self._add("CHECK-ARCH-TRACE-001", manifest_path, "source_baseline must contain at least one traceability reference")
            manifest = Manifest(
                path=manifest_path,
            ...
            if manifest.component_id in self.manifests:
                self._add("CHECK-ARCH-MANIFEST-009", manifest_path, f"duplicate component id {manifest.component_id!r}")
    ...
    def _load_owner_ids(self) -> set[str]:
        ...
        try:
            ...
            self._add("CHECK-ARCH-OWNER-002", path, f"invalid owner registry: {error}")
            return set()
        owners = payload.get("owners")
        if not isinstance(owners, list):
            self._add("CHECK-ARCH-OWNER-003", path, "owners must be an array")
            return set()
        ids = {entry.get("id") for entry in owners if isinstance(entry, dict) and isinstance(entry.get("id"), str)}
        if len(ids) != len(owners):
            self._add("CHECK-ARCH-OWNER-004", path, "every owner must have a unique string id")
        return ids
        ...
        for manifest in self.manifests.values():
            for dependency in manifest.dependencies:
                if dependency == manifest.component_id:
                    self._add("CHECK-ARCH-DEP-001", manifest.path, "component must not depend on itself")
                elif dependency not in known:
                    self._add("CHECK-ARCH-DEP-002", manifest.path, f"unknown dependency {dependency!r}")
            if len(set(manifest.dependencies)) != len(manifest.dependencies):
                self._add("CHECK-ARCH-DEP-003", manifest.path, "dependencies must not contain duplicates")
    ...
    def _check_language_boundaries(self) -> None:
        ...
        # it. This prevents structural-space manifests from overriding the
        # narrower language contract of an application or component.
        source_files = [
            path
        ...
        for path in source_files:
            ...
            if not candidates:
                # Repository support code (tests, validation and build tools)
                # is governed by its own quality gates, not by product manifests.
                continue
        ...
        for manifest in manifests:
            if manifest.lifecycle != "active" or manifest.kind not in {"application", "component"}:
                continue
            ...
            owned_sources = [
                ...
                and max(
                    (candidate for candidate in manifests if path.is_relative_to(candidate.path.parent)),
                    key=lambda item: len(item.path.parent.parts),
                ) == manifest
            ]
            if not owned_sources:
                self._add("CHECK-ARCH-CODE-001", component_dir, "active application/component must contain executable source")
    ...
    def _check_namespace(self, path: Path) -> None:
        ...
        if path.suffix == ".java":
            ...
            if not match or not match.group(1).startswith("io.infranexum"):
                self._add("CHECK-ARCH-NS-001", path, "Java package must use io.infranexum namespace")
            ...
            if legacy_import_prefix in text:
                self._add("CHECK-ARCH-NS-002", path, "Go imports must use infranexum namespace")

    def _check_forbidden_names(self) -> None:
        forbidden = ["".join(parts).lower() for parts in self.policy["forbidden_name_parts"]]
        scan_extensions = self._CODE_EXTENSIONS | {".md", ".json", ".yaml", ".yml", ".xml", ".properties"}
        for path in self.root.rglob("*"):
            ...
            for token in forbidden:
                if token in relative or token in text:
                    self._add("CHECK-BRAND-001", path, f"forbidden legacy namespace {token!r} detected")

    def _check_secrets(self) -> None:
        """Block high-confidence credential material without echoing the match."""
        extensions = {item.lower() for item in self.policy.get("secret_scan_extensions", [])}
        ...
        for path in self.root.rglob("*"):
            if not path.is_file():
                continue
            if any(part in forbidden_artifacts or part == ".git" for part in path.parts):
                continue
    ...
    def _check_artifacts(self) -> None:
        ...
        for path in self.root.rglob("*"):
            if path.name in forbidden:
                self._add("CHECK-REPO-CLEAN-001", path, f"generated or forbidden artifact {path.name!r} must not be committed")

    def _add(self, check_id: str, path: Path, message: str) -> None:
        self.violations.append(Violation(check_id, self._render_path(path), message))
